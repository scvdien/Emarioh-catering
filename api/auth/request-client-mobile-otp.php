...
$fullName = emarioh_normalize_name((string) ($data['full_name'] ?? $currentUser['full_name'] ?? 'Client'));
...
if ($fullName === '') {
    $fullName = (string) ($currentUser['full_name'] ?? 'Client');
}
...
$response = [
    'message' => emarioh_sms_gateway_is_enabled()
        ? 'OTP sent by SMS. Verify it before saving the new mobile number.'
        : 'OTP created. Verify it before saving the new mobile number.',
